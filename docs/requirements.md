...
- 每次關閉後重新打開月曆視窗，日期範圍回到今天起的 28 天，不保留上次瀏覽位置。
...
- 預設顯示從今天開始的固定 28 天。
- 使用 7 欄 × 4 列排列，每列為連續 7 天。
- 不受月份邊界或完整星期邊界限制，第一格永遠是目前範圍的起始日。
- 提供上一週、下一週控制，每次將整個 28 天範圍前移或後移 7 天。
- 提供「回到今天」，將範圍重設為今天起的 28 天。
...
- 只在本機保存偏好設定，包括：選取的行事曆、是否顯示已拒絕行程、外觀、視窗尺寸、釘選狀態及登入時啟動設定。
- 不使用 iCloud 同步偏好設定。
